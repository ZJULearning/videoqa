...
class rereading:
    def __init__(self, batch_size, output_length,
        ...
        self.toJoint = embeddingLayer(
            ...
            joint_dim)
        self.rereader = questionAttentionLayer(
            joint_dim,
            memory_dim,
            output_dim)
    ...
    def build_model(self, frame, q, q_rev, mask, maskMat, mask01, padding):
        ...
        video = videoJoint[:, -1, :]
        w_q = self.rereader.apply(video, questionJoint,
                                  maskMat, bs)
        fc_r = self.seq_gen.apply(self.output_length, video, w_q, padding)
